...
#
#The function fetch_hillenbrand() loads the database in a similar way as the databases supported by the sklearn fetch_datasets.
...
def fetch_hillenbrand(genders='adults',vowels=[],features=['f0','F1','F2','F3'],targets=[],Debug=False,Output='Pandas',return_X_y=False):
    ...
    if not set(genders) <= set(allgenders):
        print("fetch_hillenbrand(): GENDER List Error")
        return
    
    if(len(vowels) == 0):
        vowels = allvowels
    if not set(vowels) <= set(allvowels):
        print("fetch_hillenbrand(): VOWEL List Error")
        return
    
    if(len(features) ==0):
        features = allfeatures
    if not set(features) <= set(allfeatures):
        print("fetch_hillenbrand(): FEATURE List Error")
        return
    
    if(len(targets) == 0):
        targets = ['vid','gid']
    if not set(targets) <= set(alltargets):
        print("fetch_hillenbrand(): TARGET List Error")
        return
